...
type (
	AppRdKpi struct {
		...
		// 结束时间
		EndTime     string
		Coefficient AppRdCoefficient
	}

	AppRdCoefficient struct {
		...
		FIRST_BUG_RATE  float64 // 第一bug率 0
		SECOND_BUG_RATE float64 // 第二bug率 0.1
		THIRD_BUG_RATE  float64 // 第三bug率 0.2
		FORTH_BUG_RATE  float64 // 第四bug率 0.3
		FIFTH_BUG_RATE  float64 // 第五bug率 0.4
		SIXTH_BUG_RATE  float64 // 第六bug率 0.5

		FIRST_BUG_BASE  float64 // 第一bug率基数 1
		SECOND_BUG_BASE float64 // 第二bug率基数 0.9
		THIRD_BUG_BASE  float64 // 第三bug率基数 0.8
		FORTH_BUG_BASE  float64 // 第四bug率基数 0.7
		FIFTH_BUG_BASE  float64 // 第五bug率基数 0.6
		SIXTH_BUG_BASE  float64 // 第六bug率基数 0.5
		ZERO_BUG_BASE   float64 // 零bug基数 0

		TOP_COEFFICIENT    float64 // 最高系数
		SECOND_COEFFICIENT float64 // 第二系数
		THIRD_COEFFICIENT  float64 // 第三系数
		FORTH_COEFFICIENT  float64 // 第四系数
		FIFTH_COEFFICIENT  float64 // 第五系数
		SIXTH_COEFFICIENT  float64 // 第六系数
	}

	AppRdKpiResult2 struct {
...
func NewAppRdKpi(db *sql.DB, account, startTime, endTime string, coefficient AppRdCoefficient) *AppRdKpi {
	return &AppRdKpi{
		Account:     account,
		Db:          db,
		StartTime:   startTime,
...
func (l *AppRdKpi) GetAppRdKpiGrade() (result RdKpiResult2) {
	...
	for _, story := range storyMap {
		// 需求基础分
		storyBase := l.GetAppStoryBase(story.StoryEstimate, story.TaskConsumed)
	...
	// bug遗留率
	// find active bugs and resolved bugs

	// if len(result.ProjectDatas) > 0 {
	// 	for _, project := range result.ProjectDatas {
	// 		activeBugs := dbQuery.AppRdActiveBugs(l.Db, project.Root, l.Account)
	...
	// 		resolvedBugs := dbQuery.AppRdResolvedBugs(l.Db, project.Root, l.Account)
	...
	// 		project.AppBugRate = float64(len(activeBugs)) / float64(len(resolvedBugs)+len(activeBugs))
	// 		result.AppAverageBugRate += project.AppBugRate
	// 	}

	// 	result.AppAverageBugRate /= float64(len(result.ProjectDatas))
	// }

	activeBugs := dbQuery.AppRdActiveBugs(l.Db, l.Account)
	for _, bug := range activeBugs {
		result.BugDatas = append(result.BugDatas, BugData{
			BugId:         bug.BugId,
			BugTitle:      bug.BugTitle,
	...
	resolvedBugs := dbQuery.AppRdResolvedBugs(l.Db, l.Account)
	for _, bug := range resolvedBugs {
		result.BugDatas = append(result.BugDatas, BugData{
			BugId:         bug.BugId,
			BugTitle:      bug.BugTitle,
	...
	if len(activeBugs) == 0 {
		result.AppAverageBugRate = 0
	} else {
		result.AppAverageBugRate = float64(len(activeBugs)) / float64(len(resolvedBugs)+len(activeBugs))
	}



	result.BugDetail = fmt.Sprintf("项目均bug遗留率: %.2f\n\n", result.AppAverageBugRate)
	result.BugGrade = l.Coefficient.BUG_CARRY_OVER_STANDARD * l.GetAppBugRateBase(result.AppAverageBugRate)

	result.TotalGrade = result.ProjectGrade + result.StoryGrade + result.BugGrade

	result.Coefficient = l.GetAppKpiGradeStandard(result.TotalGrade)
	return result
}

func (l *AppRdKpi) MakeAppRdReport(department, career, dir, boss, path string) error {
	data := l.GetAppRdKpiGrade()
	...
	// Rename Sheet1 to "App开发部-研发岗2025年绩效考核表"
	err = f.SetSheetName("Sheet1", "App开发部-研发岗2025年绩效考核表")
	if err != nil {
	...
	f.SetCellValue("Sheet2", fmt.Sprintf("C%v", rowNum), "项目id")
	...
	for _, bug := range data.BugDatas {
		...
		f.SetCellValue("Sheet2", fmt.Sprintf("C%v", rowNum), bug.ProjectId)
		f.SetCellValue("Sheet2", fmt.Sprintf("D%v", rowNum), bug.BugStatus)
		f.SetCellValue("Sheet2", fmt.Sprintf("E%v", rowNum), bug.BugResolution)
	}

	// Rename Sheet2 to "App开发部-个人统计"
	err = f.SetSheetName("Sheet2", "App开发部-个人统计")
	if err != nil {
		return fmt.Errorf("set sheet2 name fail: %v", err)
	}
...
func (l *AppRdKpi) GetAppStoryBase(estimate, consumed float64) float64 {
	return (estimate / l.Coefficient.STORY_BASE_TIME) * l.Coefficient.STORY_BASE_SCORE
}

func (l *AppRdKpi) GetAppKpiGradeStandard(totalGrade float64) float64 {
	if totalGrade >= 90 {
...
func (l *AppRdKpi) GetAppBugRateBase(rate float64) float64 {
	if rate <= l.Coefficient.FIRST_BUG_RATE {
		return l.Coefficient.FIRST_BUG_BASE
	} else if rate > l.Coefficient.FIRST_BUG_RATE && rate <= l.Coefficient.SECOND_BUG_RATE {
		return l.Coefficient.SECOND_BUG_BASE
	} else if rate > l.Coefficient.SECOND_BUG_RATE && rate <= l.Coefficient.THIRD_BUG_RATE {
		return l.Coefficient.THIRD_BUG_BASE
	} else if rate > l.Coefficient.THIRD_BUG_RATE && rate <= l.Coefficient.FORTH_BUG_RATE {
		return l.Coefficient.FORTH_BUG_BASE
	} else if rate > l.Coefficient.FORTH_BUG_RATE && rate <= l.Coefficient.FIFTH_BUG_RATE {
		return l.Coefficient.FIFTH_BUG_BASE
	} else if rate > l.Coefficient.FIFTH_BUG_RATE && rate <= l.Coefficient.SIXTH_BUG_RATE {
		return l.Coefficient.SIXTH_BUG_BASE
	}
	return l.Coefficient.ZERO_BUG_BASE
}
